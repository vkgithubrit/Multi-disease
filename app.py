...
# # load models (assumes same filenames you used)
# diabetes_model = pickle.load(open(f'{working_dir}/notebook/diabetes_model.pkl','rb'))
# heart_disease_model = pickle.load(open(f'{working_dir}/notebook/heart_model.pkl','rb'))
# parkinsons_disease_model = pickle.load(open(f'{working_dir}/notebook/parkinsons_model.pkl','rb'))

# # initial flags (kept as in your original file)
# NewBMI_Overweight=0
# NewBMI_Underweight=0
# NewBMI_Obesity_1=0
# NewBMI_Obesity_2=0 
...
# # small helper: browser text-to-speech using Web Speech API
# def speak(text):
#     # Use a tiny HTML+JS snippet to call the browser speechSynthesis
#     # height small but nonzero so it executes
...
# # helper to render highlighted result banner at top of page
# def show_highlighted_result(text, positive=True):
...
#     ">
#       <div style="flex:1">
#         <div style="font-size:20px; font-weight:700; color:{color}; margin-bottom:6px;">Prediction Result</div>
#         <div style="font-size:18px; color:#111;">{text}</div>
#       </div>
#       <div style="background:{color}; color:white; padding:12px 18px; border-radius:10px; font-weight:700;">
#         {text}
#       </div>
#     </div>
#     """
...
#     # Added placeholders so each box clearly tells what to enter
#     with col1:
...
#         # Ensure we pass the shape the model expects (safe adapt)
#         expected = getattr(diabetes_model, "n_features_in__", None)
#         try:
#             if expected is None:
#                 # try raw first, fallback to full
#                 X_try = np.array(raw_features, dtype=float).reshape(1, -1)
#                 try:
#                     pred = diabetes_model.predict(X_try)
#                     used = "raw"
...
#                     raise ValueError(
#                         f"Model expects {expected} features. Raw features = {len(raw_features)}, "
#                         f"Full (raw+engineered) = {len(full_input)}. Retrain model or change inputs."
#                     )
#         except Exception as e:
#             st.error("Prediction failed: " + str(e))
#             st.stop()
...
#         # Show highlighted result and speak
#         show_highlighted_result(diabetes_result, positive=positive)
#         speak(f"Diabetes prediction complete. {diabetes_result}")

#     st.success("")  # keep page layout stable
...
#     # placeholders added (enter the box name)
#     with col1:
#         age = st.text_input("Age", placeholder="Enter age in years e.g. 54")
#     with col2:
#         sex = st.text_input("Sex", placeholder="Enter sex encoded as 1=male, 0=female (e.g. 1)")
#     with col3:
#         cp = st.text_input("Chest Pain Types", placeholder="Enter chest pain type code (e.g. 1)")
#     with col1:
#         trtbps = st.text_input("Resting Blood Pressure", placeholder="Enter resting BP (mm Hg) e.g. 130")
#     with col2:
#         chol = st.text_input("Serum Cholestroal in mg/dl", placeholder="Enter cholesterol mg/dl e.g. 233")
#     with col3:
#         fbs = st.text_input('Fasting Blood Sugar > 120 mg/dl', placeholder="Enter 1 if true else 0")
#     with col1:
#         restecg = st.text_input('Resting Electrocardiographic results', placeholder="Enter 0,1,2")
#     with col2:
#         thalachh = st.text_input('Maximum Heart Rate achieved (thalach)', placeholder="Enter e.g. 150")
#     with col3:
#         exng = st.text_input('Exercise Induced Angina', placeholder="Enter 1 if yes else 0")
#     with col1:
#         oldpeak = st.text_input('ST depression (oldpeak)', placeholder="Enter float e.g. 1.2")
#     with col2:
#         slp = st.text_input('Slope of the peak exercise ST segment', placeholder="Enter 0/1/2")
#     with col3:
#         caa = st.text_input('Major vessels colored by flourosopy (0-3)', placeholder="Enter 0,1,2,3")
#     with col1:
#         thall = st.text_input('thal (0 normal; 1 fixed; 2 reversible)', placeholder="Enter 0,1 or 2")

#     heart_disease_result = ""
#     if st.button("Heart Disease Test Result"):
#         # Defensive conversion for heart inputs
#         try:
#             v_age = float(age)
#             v_sex = float(sex)
#             v_cp = float(cp)
#             v_trtbps = float(trtbps)
#             v_chol = float(chol)
#             v_fbs = float(fbs)
#             v_restecg = float(restecg)
#             v_thalachh = float(thalachh)
#             v_exng = float(exng)
#             v_oldpeak = float(oldpeak)
#             v_slp = float(slp)
#             v_caa = float(caa)
#             v_thall = float(thall)
#         except Exception as e:
#             st.error("Please fill all heart fields with valid numeric values. Error: " + str(e))
#             st.stop()

#         user_input = [v_age, v_sex, v_cp, v_trtbps, v_chol, v_fbs, v_restecg,
#                       v_thalachh, v_exng, v_oldpeak, v_slp, v_caa, v_thall]

#         # Validate expected features before predict
#         expected_h = getattr(heart_disease_model, "n_features_in__", None)
#         try:
#             Xh = np.array(user_input, dtype=float).reshape(1, -1)
...
#         show_highlighted_result(heart_disease_result, positive=positive)
#         speak(f"Heart disease prediction complete. {heart_disease_result}")
...
#     # placeholders added for each input
#     with col1:
#         MDVPFOHZ = st.text_input('MDVP:Fo(Hz)', placeholder="Enter MDVP Fo (Hz)")
#     with col2:
#         MDVPFHIHZ = st.text_input('MDVP:Fhi(Hz)', placeholder="Enter MDVP Fhi (Hz)")
#     with col3:
#         MDVPFLOHZ = st.text_input('MDVP:Flo(Hz)', placeholder="Enter MDVP Flo (Hz)")
#     with col4:
#         MDVPJITTERPERCENTAGE = st.text_input('MDVP:Jitter(%)', placeholder="Enter Jitter %")
#     with col1:
#         MDVPJITTERPERABS = st.text_input('MDVP:Jitter(Abs)', placeholder="Enter Jitter Abs")
#     with col2:
#         MDVPRAP = st.text_input('MDVP:RAP', placeholder="Enter MDVP RAP")
#     with col3:
#         MDVPPPQ = st.text_input('MDVP:PPQ', placeholder="Enter MDVP PPQ")
#     with col4:
#         JITTERDDP = st.text_input('Jitter:DDP', placeholder="Enter Jitter DDP")
#     with col1:
#         MDVPSHIMMER = st.text_input('MDVP:Shimmer', placeholder="Enter MDVP Shimmer")
#     with col2:
#         MDVPSHIMMERDB = st.text_input('MDVP:Shimmer(dB)', placeholder="Enter Shimmer dB")
#     with col3:
#         SHIMMERAPQ3 = st.text_input('Shimmer:APQ3', placeholder="Enter APQ3")
#     with col4:
#         SHIMMERAPQ5 = st.text_input('Shimmer:APQ5', placeholder="Enter APQ5")
#     with col1:
#         MDVPAPQ = st.text_input('MDVP:APQ', placeholder="Enter MDVP APQ")
#     with col2:
#         SHIMMERDDA = st.text_input('Shimmer:DDA', placeholder="Enter Shimmer DDA")
#     with col3:
...
#     if st.button("Parkinsons's Test Result"):
#         # Defensive conversion for all inputs
#         try:
#             values = [
#                 MDVPFOHZ, MDVPFHIHZ, MDVPFLOHZ, MDVPJITTERPERCENTAGE,
#                 MDVPJITTERPERABS, MDVPRAP, MDVPPPQ, JITTERDDP, MDVPSHIMMER,
...
#                 SHIMMERDDA, NHR, HNR, RPDE, DFA, SPREAD1, SPREAD2, D2, PPE
#             ]
#             numeric_vals = [float(x) for x in values]
#         except Exception as e:
#             st.error("Please fill all Parkinsons fields with valid numeric values. Error: " + str(e))
#             st.stop()

#         Xp = np.array(numeric_vals, dtype=float).reshape(1, -1)
...
#         show_highlighted_result(parkinsons_result, positive=positive)
#         speak(f"Parkinsons prediction complete. {parkinsons_result}")
